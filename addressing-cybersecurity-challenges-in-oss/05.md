...
至少有 10 个工具类别专注于解决 OSS 安全问题。在调查选项中，组织平均使用 2.8 个安全工具类别。静态代码分析SCA 和 静态态应用程序安全测试 SAST 工具是解决 OSS 安全问题中最常用的工具（见图 14）。使用 IaC 工具（间接解决安全问题）和 Web 应用程序扫描器（动态应用程序安全测试 DAST 类别的一部分）是许多组织使用的其他工具。
