...
Open source software (OSS) has had a tremendous impact on the
...
A worldwide survey was fielded in April 2022, targeting the
following roles
...
error (MoE) is +/- 3.6% at a 90% confidence levelG
```
```
...
discussed in a separate report to be published in 2022 Q3G
```
```
...
Having a security policy covering OSS indicates that you have a
...
by organization size as follows: small organizations (44%, 1-
...
difference in distributions between organizations with 1-
...
```
One reason that small organizations are OSS security challenged
...
```
A simple weighted average of all responses shows a composite
...
```
The similarity of this distribution when compared to Figure 4
...
```
```
Yes No Total Who is responsible for defining your open
...
```
What is known is that organizations are not well-positioned to
...
(49 dependencies), Go (56 dependencies), or Java (
...
Recent efforts by the US Government to encourage, and even mandate, organizations to create a Software Bill of Materials (SBOM) is evidence
...
```
A large part of the value of SCA tools is finding where
...
### How organizations are addressing and
...
Organizational approaches to identifying vulnerabilities in
...
```
```
We don’t Other Don’t Know
```
```
Source: 2022 Open Source Supply Chain Security Survey.
```

The second leading approach practiced by 33% of organizations,
...
```
How do you check the security of the open source packages
that you use? (select all that apply)
```
```
44%
40%
...
What security tools do you regularly use when developing

open source software? (select all that apply) by Do you have

an open source security policy in place for open source
...
there are already several trusted sources who can address this needa
```
```
RThere are a variety of sources to identify best practices/
...
SLSA (<https://slsa.dev>)X
```
```
RThis also suggests an interest in encouraging developers to learn
...
important to improving the security
of the open source software supply
chain? (select all that apply)
```
```
In third place for most popular activities around secure software development, we see a tie between
...
```
What are the three most important ways that open source project
...
```
1 Secure OSS production. Focus on preventing security defects and
vulnerabilities in code and open source packages in the first
placeT
```
```
1 Improving vulnerability discovery and remediation. Improving the
process for finding defects and fixing themT
```
```
1 Shorten ecosystem patching response time. Shorten the response
...
Collaborate with vendors to create more intelligent security
...
The objective of this research was to understand the following1
...
Foundation where he is the principal investigator on a variety of
research projects core to the Linux Foundation’s understanding of how
open source software is an engine of innovation for producers and
consumers of information technology. Steve specializes in primary
research techniques developed over 30 years as a software industry
analyst. Steve is a subject matter expert in application development
and deployment topics including DevOps, application management, and
decision analytics. Steve brings experience in a variety of
quantitative and qualitative research techniques that enable deep
insight into market dynamics and has pioneered research across many
application development and deployment domains. Steve has authored
over 1,000 publications and provided market guidance through
syndicated research and custom consulting to the world’s leading
software vendors and high-profile startups.





Stephen Hendrick Martin Mckeay

```
Martin Mckeay is Snyk’s Senior Editorial Research Manager,
where he works with teams across the company to build
reports that increase the knowledge base of security
professionals and developers. With over twenty years as a
security professional, Martin started his career in help
desk operations, continuously building to more complex and
diverse roles over the years. Over the last seven years,
Martin has developed the skills to turn data into
intelligence and translate ‘geek speak’ into language
...
This document was authored with the support and collaboration of the
following individuals and organizations: Stephen Augustus (Cisco),
Brian Behlendorf (Linux Foundation), Hilary Carter (Linux Foundation),
Randall Degges (Snyk), Brian Demers, Michael Dolan (Linux Foundation),
Kim Lewandowski (Chainguard), Oleg Nenashev (Dynatrace), Mike
Milinkovich (Eclipse Foundation), Megan Moore (Synk), Nick O’Leary
(FlowForge), Christina Oliviero (Linux Foundation), Ashwin Ramaswami
(Plaintext Group), Clark Roundy (Eclipse Foundation), Jed Salazar
(Chainguard), Melissa Schmidt (Linux Foundation), Robert Scholte
(Apache), Micah Silverman (Snyk), Daniel Stenberg (WolfSSL), Kate
Stewart (Linux Foundation), Liran Tal (Synk), Adolfo Garcia Veytia
(Chainguard), Derek Weeks (Linux Foundation), David A. Wheeler (Linux
Foundation), Sarah Wills (Snyk).
...
scale of open source collaboration, providing insight into
emerging technology trends, best practices, and the global
impact of open source projects. Through leveraging project
databases and networks, and a commitment to best practices in
quantitative and qualitative methodologies, Linux Foundation
Research is creating the go-to library for open source insights
for the benefit of organizations the world over.


```
Snyk is a developer-first security company that helps
software-driven businesses develop fast and build
securely. Snyk provides a platform to secure all of the
critical components of today’s cloud native application
development. Snyk is securing the industry leaders such as
Google, Salesforce, Asos, BBC, and Asurion. For more
information or to get started with Snyk for free, visit
https://snyk.io
```
...
To reference this work, please cite as follows: Stephen Hendrick
and Martin Mckeay, “Addressing Cybersecurity Challenges in open
source Software,” foreword by Brian Behlendorf, Linux Foundation
and Snyk, June 2022
